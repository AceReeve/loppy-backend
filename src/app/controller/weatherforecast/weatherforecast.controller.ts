...
import {
  WeatherForecastNotificationDTO,
  WeatherForecastUpdateDTO,
} from 'src/app/dto/weatherforecast';
...
export class WeatherForecastController {
  ...
  async addWeatherForecastNotificationData(
    @Req() request,
    @Body() weatherForecastDTO: WeatherForecastNotificationDTO,
  ) {
    const userId = request.user.sub;
    const response =
      await this.weatherNotificationService.addWeatherForecastNotificationData(
        userId,
  ...
  async updateWeatherForecastNotificationLocation(
    @Req() request,
    @Body() weatherForecastUpdateDTO: WeatherForecastUpdateDTO,
  ) {
    const userId = request.user.sub;
    const response =
      await this.weatherNotificationService.addWeatherForecastNotificationLocation(
        userId,
  ...
  async getNotificationData(@Req() request): Promise<object> {
    const userId = request.user.sub;
    const response =
      await this.weatherNotificationService.getWeatherNotificationData(userId);
  ...
  async getWeatherAPIforecast(@Query() queries: ExpressQuery): Promise<object> {
    const response = await this.weatherService.weatherAPIforecast(
      String(queries.city),
      Number(queries.days),
    );
  ...
  async getDayOpenweatherByCity(
    @Query() queries: ExpressQuery,
  ): Promise<object> {
    const response = await this.weatherService.openWeatherDaily(
      String(queries.city),
    );
  ...
  async getDailyyOpenweatherByCity(
    ...
    const response = await this.weatherService.openWeatherDay(
      String(queries.city),
    );
