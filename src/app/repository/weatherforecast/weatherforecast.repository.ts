...
export class WeatherForecastRepository {
  constructor(
    ...
    private weatherForecastModel: Model<WeatherForecast & Document>,
  ) {}
  ...
  async addWeatherForecastNotificationData(
    ...
    const isExisting = await this.weatherForecastModel.findOne({
      user_id: user_id,
    });
    if (isExisting) {
      const weatherforecastLocation =
        await this.weatherForecastModel.findOneAndUpdate(
          { user_id: user_id },
          {
            $set: {
  ...
  async getWeatherNotificationData(user_id: string): Promise<any> {
    const weatherforecastLocation = await this.weatherForecastModel.findOne({
      user_id: user_id,
    });
    return weatherforecastLocation;
  }
