...
export class UserRepository implements AbstractUserRepository {
  ...
  private async getImageInfo(
    ...
    const findNonCatalogLineItem = await this.userInfoModel.findOne({
      _id: new Types.ObjectId(id),
    });

    if (!findNonCatalogLineItem)
      throw new BadRequestException(`Invalid non-catalog line item id: ${id}`);

    const { profile } = findNonCatalogLineItem;
